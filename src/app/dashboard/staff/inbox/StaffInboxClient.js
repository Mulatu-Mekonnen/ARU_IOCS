...
export default function StaffInboxClient() {
  ...
  async function loadInbox() {
    ...
    setError("");
    try {
      const query = new URLSearchParams({ inbox: "true", page: "1", pageSize: "200" });
      const res = await fetch(`/api/dashboard/staff/agendas?${query.toString()}`, { credentials: "include" });
      if (!res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-gray-500">Communications received by your office.</p>
        </div>
        <input
          value={search}
        ...
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="p-6 text-center text-sm text-gray-500">No messages found.</td>
              </tr>
            ) : (
              filtered.map((agenda) => (
                <tr key={agenda.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm text-gray-800">{agenda.title}</td>
                  <td className="px-4 py-3 text-sm text-gray-800">{agenda.createdBy?.name || "Unknown"}</td>
                  <td className="px-4 py-3 text-sm text-gray-800">{agenda.senderOffice?.name || "-"}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-800 space-x-2">
                    ...
                    <button className="text-gray-500 hover:text-gray-700">Acknowledge</button>
                  </td>
                </tr>
              ))
            )}
